...
class EntryTest {
    ...
    fun `parseLine should correctly parse a valid entry with tags`() {
        ...
        val entry = Entry.parseLine(line)
        assertEquals("metric", entry.name)
        assertEquals(listOf(Tag("name", "value"), Tag("tag1", "value1")), entry.tags)
    ...
    fun `parseLine should handle single tag`() {
        ...
        val entry = Entry.parseLine(line)
        assertEquals("metric", entry.name)
        assertEquals(listOf(Tag("tag", "value")), entry.tags)
    ...
    fun `parseLine should handle multiple tags in different order`() {
        ...
        val entry = Entry.parseLine(line)
        assertEquals("metric", entry.name)
        assertEquals(listOf(Tag("tag1", "value1"), Tag("tag2", "value2")), entry.tags)
    ...
    fun `parseLine should handle tags with empty values`() {
        ...
        assertEquals("metric", entry.name)
        assertEquals(
            listOf(Tag("tag1", ""), Tag("tag2", "value2")),
            entry.tags
        )
    ...
    fun `parseLine should handle tags with leading and trailing spaces in values`() {
        ...
        assertEquals("metric", entry.name)
        assertEquals(
            listOf(Tag("tag1", "  leading space"), Tag("tag2", "trailing space  ")),
            entry.tags
        )
